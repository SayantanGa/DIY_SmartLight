...
int ConstantBrightness(int x){  //For constant brightness output
	return 255*x/9.0;
}

int LogBrightness(float x){
...
class LED{  //class to store the variables and functions corresponding to the operating light
    public:
        ...
        void update(){  //Updates the Light brightness and temperature according to LUX and User input
		    ...
		    void update_with_const_brightness(int level){
...
void loop(){
    ...
    if(data[0]==1){
      Light.glow();
      if(data[1]==0)
        Light.update_with_const_brightness(int(data[4]));
      if(data[1]==1)
        Light.update();
      if(data[3]==1)
        data[3] = Light.Hotter();
